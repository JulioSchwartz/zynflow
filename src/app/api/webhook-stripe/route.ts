  ...
  switch (event.type) {
    ...
    case 'checkout.session.completed': {
      ...
      await sb.from('usuarios_flow').update({
        ...
        stripe_price_id:        'price_1TTQTZPI61I7rxR2xph7S0Ht',
        subscription_ends_at:   endsAt,
      }).eq('user_id', userId)

      // Notificação interna
      try {
        const { data: u } = await sb.from('usuarios_flow').select('email, nome').eq('user_id', userId).single()
        await resend.emails.send({
          from: 'noreply@zynplan.com.br',
          to: 'j.ulioschwartz@hotmail.com',
          subject: '💳 Nova assinatura Zynflow!',
          html: `<p><strong>${u?.nome}</strong> (${u?.email}) assinou o Zynflow Pro!</p>`,
        })
      } catch (_) {}
      break
    }
    ...
    case 'customer.subscription.trial_will_end': {
      ...
      const { data: u } = await sb.from('usuarios_flow')
        .select('email, nome').eq('stripe_customer_id', custId).single()
      ...
      if (u?.email) {
        try {
          await resend.emails.send({
            from: 'noreply@zynplan.com.br',
            to: u.email,
            subject: '⏰ Seu trial do Zynflow termina em 3 dias',
            html: `
              <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#07080F;color:#E8EAF2;padding:40px;border-radius:16px;">
                <div style="text-align:center;margin-bottom:32px;">
                  ...
                  <p style="color:#6B7280;margin:0;">Zynflow — Controle Financeiro do Autônomo</p>
                </div>
                <p style="color:#9CA3AF;line-height:1.7;">Olá, <strong style="color:#fff;">${u.nome}</strong>!</p>
                <p style="color:#9CA3AF;line-height:1.7;">Seu período gratuito de 30 dias no Zynflow termina em <strong style="color:#fff;">3 dias</strong>. Para continuar tendo acesso ao Método 3 Passos, teto semanal automático e Fundo de Meses Fracos, ative sua assinatura por apenas <strong style="color:#818CF8;">R$ 19,90/mês</strong>.</p>
