...
// PF não tem "meses_fracos" — apenas emergência, investimento e previdência
const TIPOS_INFO: Record<string, { label: string; desc: string; icon: string; cor: string }> = {
  ...
  investimento: { label: 'Investimentos',         desc: 'Invista após construir a reserva de emergência. CDB, Tesouro Direto, previdência.', icon: '📈', cor: VERDE },
...
export default function ReservasPFClient() {
  ...
  async function salvarEditar() {
    if (!editando) return
    setSalvando(true)
    await supabase.from('reservas_flow').update({ nome: formNome, percentual: formPct, meta: formMeta }).eq('id', editando.id)
    setReservas(prev => prev.map(r => r.id === editando.id ? { ...r, nome: formNome, percentual: formPct, meta: formMeta } : r))
    setSalvando(false)
  ...
  async function confirmarAporte() {
    ...
    await Promise.all([
      ...
      supabase.from('aportes_reservas_flow').insert({ user_id: userId, reserva_id: modalAporte.id, valor: aporteValor, data: aporteData }),
    ])
  ...
  async function abrirHistorico(r: Reserva) {
    ...
    const { data } = await supabase.from('aportes_reservas_flow').select('id, valor, data').eq('reserva_id', r.id).order('data', { ascending: false })
    setHistorico(data || [])
  ...
  async function adicionarTipo(tipo: string) {
    ...
    const { data } = await supabase.from('reservas_flow').insert({ user_id: userId, tipo, percentual: 5, valor_acumulado: 0, meta: 0 }).select().single()
  ...
  async function criarReservaPersonalizada() {
    ...
    const { data } = await supabase.from('reservas_flow').insert({
      user_id: userId, tipo: `custom_${Date.now()}`, nome: novaNome, percentual: novaPct, valor_acumulado: 0, meta: novaMeta,
    }).select().single()
    if (data) setReservas(prev => [...prev, data])
    setSalvandoNova(false)
    setModalNova(false)
    setNovaNome(''); setNovaPct(5); setNovaMeta(0)
  }
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {desc && (
                  <button onClick={e => { e.stopPropagation(); setInfoAberto(isInfoAberto ? null : r.id) }}
              ...
              {isInfoAberto && desc && (
                <div style={{ fontSize: 12, color: '#818CF8', marginTop: 4, maxWidth: 340, lineHeight: 1.5, background: 'rgba(79,70,229,0.08)', padding: '6px 10px', borderRadius: 8, border: '1px solid rgba(79,70,229,0.2)' }}>{desc}</div>
              )}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }} onClick={e => e.stopPropagation()}>
            <button onClick={() => abrirAporte(r)} style={{ background: `${cor}20`, border: `1px solid ${cor}40`, borderRadius: 8, padding: '7px 14px', fontSize: 13, color: cor, cursor: 'pointer', fontWeight: 600 }}>+ Aporte</button>
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, color: '#fff', margin: 0 }}>Reservas</h1>
          <p style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>Pague-se primeiro. Construa sua segurança financeira.</p>
        </div>
        <button onClick={() => setModalNova(true)} style={{ background: INDIGO, color: '#fff', border: 'none', borderRadius: 8, padding: '9px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          + Nova reserva
        </button>
      </div>
      ...
      <div style={{ background: 'rgba(79,70,229,0.08)', border: '1px solid rgba(79,70,229,0.2)', borderRadius: 12, padding: '14px 18px', marginBottom: 24, fontSize: 13, color: '#818CF8' }}>
        💡 Assim que receber seu salário, transfira antes de gastar: primeiro a reserva de emergência, depois investimentos e previdência.
      </div>
      ...
      {modalEditar && editando && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 440 }}>
            ...
            <div style={{ marginBottom: 16 }}><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Nome</label><input value={formNome} onChange={e => setFormNome(e.target.value)} style={inp} /></div>
            <div style={{ marginBottom: 16 }}><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>% da receita mensal</label><input type="number" value={formPct} onChange={e => setFormPct(parseFloat(e.target.value) || 0)} style={inp} /></div>
            <div style={{ marginBottom: 24 }}><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Meta total (R$)</label><input type="number" value={formMeta} onChange={e => setFormMeta(parseFloat(e.target.value) || 0)} style={inp} /></div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={salvarEditar} disabled={salvando} style={{ flex: 1, background: INDIGO, border: 'none', borderRadius: 8, padding: 12, fontSize: 14, fontWeight: 600, color: '#fff', cursor: 'pointer', opacity: salvando ? 0.7 : 1 }}>{salvando ? 'Salvando...' : 'Salvar'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalAporte && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 400 }}>
            ...
            <p style={{ fontSize: 13, color: '#6B7280', marginBottom: 24 }}>{modalAporte.nome || TIPOS_INFO[modalAporte.tipo]?.label} — acumulado: {fmt(modalAporte.valor_acumulado)}</p>
            <div style={{ marginBottom: 16 }}><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Valor do aporte (R$)</label><input type="number" value={aporteValor} onChange={e => setAporteValor(parseFloat(e.target.value) || 0)} autoFocus style={inp} /></div>
            <div style={{ marginBottom: 24 }}><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Data</label><input type="date" value={aporteData} onChange={e => setAporteData(e.target.value)} style={inp} /></div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={confirmarAporte} disabled={salvandoAporte} style={{ flex: 1, background: INDIGO, border: 'none', borderRadius: 8, padding: 12, fontSize: 14, fontWeight: 600, color: '#fff', cursor: 'pointer', opacity: salvandoAporte ? 0.7 : 1 }}>{salvandoAporte ? 'Salvando...' : 'Confirmar aporte'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalHistorico && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 440, maxHeight: '80vh', overflowY: 'auto' }}>
            <h2 style={{ fontSize: 18, fontWeight: 600, color: '#fff', marginBottom: 4 }}>{modalHistorico.nome || TIPOS_INFO[modalHistorico.tipo]?.label}</h2>
            <p style={{ fontSize: 13, color: '#6B7280', marginBottom: 24 }}>Histórico de aportes</p>
            {loadingHistorico ? <p style={{ color: '#6B7280', textAlign: 'center' }}>Carregando...</p>
            : historico.length === 0 ? <p style={{ color: '#6B7280', textAlign: 'center', padding: '24px 0' }}>Nenhum aporte registrado ainda.</p>
            : (
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
                {historico.map(a => (
                  <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)' }}>
                    <span style={{ fontSize: 13, color: '#9CA3AF' }}>{new Date(a.data + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
            ...
            <button onClick={() => setModalHistorico(null)} style={{ width: '100%', marginTop: 20, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: 12, fontSize: 14, color: '#9CA3AF', cursor: 'pointer' }}>Fechar</button>
          </div>
        </div>
      )}
      ...
      {modalNova && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 32, width: '100%', maxWidth: 440 }}>
            ...
            <div style={{ marginBottom: 16 }}><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Nome da reserva *</label><input value={novaNome} onChange={e => setNovaNome(e.target.value)} placeholder="Ex: Férias, Carro novo..." style={inp} /></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>% da receita mensal</label><input type="number" value={novaPct} onChange={e => setNovaPct(parseFloat(e.target.value) || 0)} style={inp} /></div>
              <div><label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Meta total (R$)</label><input type="number" value={novaMeta} onChange={e => setNovaMeta(parseFloat(e.target.value) || 0)} style={inp} /></div>
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 8 }}>Ícone</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const }}>
                {['💼','🏠','🚗','✈️','🎓','💍','🎯','📱','🌴','💡','🏋️','🎸'].map(ic => (
                  <button key={ic} onClick={() => setNovaIcon(ic)} style={{ width: 36, height: 36, borderRadius: 8, border: `2px solid ${novaIcon === ic ? INDIGO : 'rgba(255,255,255,0.1)'}`, background: novaIcon === ic ? 'rgba(79,70,229,0.15)' : 'transparent', fontSize: 18, cursor: 'pointer' }}>{ic}</button>
                ))}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={criarReservaPersonalizada} disabled={salvandoNova} style={{ flex: 1, background: INDIGO, border: 'none', borderRadius: 8, padding: 12, fontSize: 14, fontWeight: 600, color: '#fff', cursor: 'pointer', opacity: salvandoNova ? 0.7 : 1 }}>{salvandoNova ? 'Criando...' : 'Criar reserva'}</button>
            </div>
          </div>
        </div>
      )}
