...
interface Ativo { id: string; ticker: string; tipo: string; nome: string | null }
interface Operacao {
...
interface CheckItem { id: string; texto: string; feito: boolean }

function calcularPosicoes(operacoes: Operacao[]) {
  const pos: Record<string, { qtd: number; custoTotal: number; ticker: string }> = {}
...
export default function InvestimentosPFClient() {
  const [userId, setUserId]       = useState<string | null>(null)
  const [ativos, setAtivos]       = useState<Ativo[]>([])
  const [operacoes, setOperacoes] = useState<Operacao[]>([])
  const [proventos, setProventos] = useState<Provento[]>([])
  const [loading, setLoading]     = useState(true)
  const [aba, setAba]             = useState<'carteira'|'ativos'|'operacoes'|'proventos'|'ir'|'notas'>('carteira')
  ...
  // Cadastro de ativo com posição inicial
  const [formAtivo, setFormAtivo] = useState({ ticker: '', tipo: 'Ação', nome: '', qtdInicial: 0, precoMedioInicial: 0, dataInicial: hoje.toISOString().split('T')[0] })
  ...
  useEffect(() => {
    async function carregar() {
      ...
      const [a, o, p, n] = await Promise.all([
        ...
        supabase.from('notas_investimentos_flow').select('*').eq('user_id', user.id).order('criado_em', { ascending: true }),
      ])
      setAtivos(a.data || [])
      setOperacoes(o.data || [])
      setProventos(p.data || [])
      // Separar checklist de anotação livre
      const itens = (n.data || []).filter((x: any) => x.tipo === 'check')
      const anotacao = (n.data || []).find((x: any) => x.tipo === 'anotacao')
      setCheckItems(itens.map((x: any) => ({ id: x.id, texto: x.texto, feito: x.feito })))
      setAnotacaoLivre(anotacao?.texto || '')
      setLoading(false)
    }
    carregar()
  }, [])
  ...
  // Lista de todos os ativos cadastrados (com posição calculada, mesmo qtd=0)
  const todosAtivos = useMemo(() => {
    return ativos.map(a => {
      const pos = posicoes[a.ticker] || { qtd: 0, custoTotal: 0 }
      const custoMedio = pos.qtd > 0 ? pos.custoTotal / pos.qtd : 0
      return { ...a, qtd: pos.qtd, custoMedio }
    })
  }, [ativos, posicoes])
  ...
  async function salvarAtivo() {
    ...
    if (novoAtivo) {
      setAtivos(prev => [...prev, novoAtivo].sort((a, b) => a.ticker.localeCompare(b.ticker)))

      // Se informou posição inicial, cria operação de compra
      if (formAtivo.qtdInicial > 0 && formAtivo.precoMedioInicial > 0) {
        const { data: op } = await supabase.from('operacoes_flow').insert({
          user_id: userId,
          ativo_id: novoAtivo.id,
          ticker,
          tipo: 'compra',
          quantidade: formAtivo.qtdInicial,
          preco_unitario: formAtivo.precoMedioInicial,
          valor_total: formAtivo.qtdInicial * formAtivo.precoMedioInicial,
          data: formAtivo.dataInicial,
          corretora: null,
          obs: 'Posição inicial',
        }).select().single()
    ...
    setSalvando(false)
    setModalAtivo(false)
    setFormAtivo({ ticker: '', tipo: 'Ação', nome: '', qtdInicial: 0, precoMedioInicial: 0, dataInicial: hoje.toISOString().split('T')[0] })
  }
  ...
  async function salvarOperacao() {
    ...
    if (!ativoId) {
      ...
      if (existe) { ativoId = existe.id }
      else {
        const { data: novoAtivo } = await supabase.from('carteira_flow').insert({ user_id: userId, ticker, tipo: 'Ação', nome: null }).select().single()
        if (novoAtivo) { setAtivos(prev => [...prev, novoAtivo]); ativoId = novoAtivo.id }
      }
    }
    const payload = { user_id: userId, ativo_id: ativoId, ticker: formOp.ticker.toUpperCase().trim(), tipo: formOp.tipo, quantidade: formOp.quantidade, preco_unitario: formOp.preco_unitario, data: formOp.data, corretora: formOp.corretora || null, obs: formOp.obs || null }
    if (editandoOp) {
      ...
      const { data } = await supabase.from('operacoes_flow').insert({ ...payload, valor_total: formOp.quantidade * formOp.preco_unitario }).select().single()
  ...
  async function salvarProvento() {
    ...
    setSalvando(true)
    const payload = { user_id: userId, ticker: formProv.ticker.toUpperCase().trim(), tipo: formProv.tipo, valor_por_cota: formProv.valor_por_cota, quantidade: formProv.quantidade, data_pagamento: formProv.data_pagamento }
    if (editandoProv) {
      const { data } = await supabase.from('proventos_flow').update({ ...payload, valor_total: formProv.valor_por_cota * formProv.quantidade }).eq('id', editandoProv.id).select().single()
      if (data) setProventos(prev => prev.map(p => p.id === editandoProv.id ? data : p))
    } else {
      const { data } = await supabase.from('proventos_flow').insert({ ...payload, valor_total: formProv.valor_por_cota * formProv.quantidade }).select().single()
  ...
  async function excluirAtivo(id: string, ticker: string) {
    if (!confirm(`Excluir ${ticker} da lista? As operações vinculadas serão mantidas.`)) return
    await supabase.from('carteira_flow').delete().eq('id', id)
  ...
  // Checklist
  async function adicionarCheckItem() {
    if (!novaNotaTxt.trim() || !userId) return
    setSalvandoNota(true)
    const { data } = await supabase.from('notas_investimentos_flow').insert({
      user_id: userId, tipo: 'check', texto: novaNotaTxt.trim(), feito: false,
    }).select().single()
    if (data) setCheckItems(prev => [...prev, { id: data.id, texto: data.texto, feito: data.feito }])
    setNovaNotaTxt('')
    setSalvandoNota(false)
  }

  async function toggleCheckItem(item: CheckItem) {
    await supabase.from('notas_investimentos_flow').update({ feito: !item.feito }).eq('id', item.id)
    setCheckItems(prev => prev.map(c => c.id === item.id ? { ...c, feito: !c.feito } : c))
  }

  async function excluirCheckItem(id: string) {
    await supabase.from('notas_investimentos_flow').delete().eq('id', id)
  ...
  async function salvarAnotacao() {
    ...
    // Upsert: busca registro tipo 'anotacao' do user e atualiza ou insere
    const { data: existe } = await supabase.from('notas_investimentos_flow')
      .select('id').eq('user_id', userId).eq('tipo', 'anotacao').maybeSingle()
    if (existe) {
      await supabase.from('notas_investimentos_flow').update({ texto: anotacaoLivre }).eq('id', existe.id)
    } else {
      await supabase.from('notas_investimentos_flow').insert({ user_id: userId, tipo: 'anotacao', texto: anotacaoLivre, feito: false })
    }
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div className="inv-abas">
        {(['carteira','ativos','operacoes','proventos','ir','notas'] as const).map(a => (
          <button key={a} className={`inv-aba${aba === a ? ' ativa' : ''}`} onClick={() => setAba(a)}>
            {a === 'carteira' ? '📊 Carteira' : a === 'ativos' ? '📋 Ativos' : a === 'operacoes' ? '🔄 Operações' : a === 'proventos' ? '💰 Proventos' : a === 'ir' ? '📋 IR' : '📝 Notas'}
          </button>
        ))}
      </div>
        ...
        <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14 }}>
          ...
          {carteira.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 24px' }}>
              ...
              <p style={{ color: '#6B7280', fontSize: 14, marginBottom: 16 }}>Nenhuma posição em carteira. Cadastre um ativo com posição inicial ou lance uma operação de compra.</p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                <button onClick={() => setModalAtivo(true)} style={{ background: INDIGO, color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>+ Cadastrar ativo</button>
                <button onClick={() => setModalOp(true)} style={{ background: 'rgba(255,255,255,0.05)', color: '#9CA3AF', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>+ Nova operação</button>
              </div>
            </div>
          ) : (
                ...
                <tbody>
                  {carteira.map(a => (
                    <tr key={a.id}>
                      ...
                      <td><button onClick={() => { setModalPreco(a.ticker); setPrecoTemp(a.precoAtual) }} className="inv-btn" style={{ fontSize: 11 }}>✏️</button></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
      ...
      {/* ── ABA ATIVOS CADASTRADOS ── */}
      {aba === 'ativos' && (
        <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14 }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 13, fontWeight: 500, color: '#fff' }}>Ativos cadastrados ({ativos.length})</span>
            <button onClick={() => setModalAtivo(true)} style={{ background: INDIGO, color: '#fff', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>+ Cadastrar ativo</button>
          </div>
          {ativos.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 24px', color: '#6B7280' }}>Nenhum ativo cadastrado ainda.</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table className="inv-table">
                <thead>
                ...
                <tbody>
                  {todosAtivos.map(a => (
                    <tr key={a.id}>
                      <td style={{ fontWeight: 700, color: '#fff' }}>{a.ticker}</td>
                      <td style={{ color: '#9CA3AF', fontSize: 12 }}>{a.nome || '—'}</td>
                      <td><span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 100, background: 'rgba(79,70,229,0.15)', color: '#818CF8' }}>{a.tipo}</span></td>
                      <td style={{ textAlign: 'right' }}>{fmtN(a.qtd, a.tipo === 'Cripto' ? 8 : 0)}</td>
                      <td style={{ textAlign: 'right' }}>{a.custoMedio > 0 ? fmt(a.custoMedio) : '—'}</td>
                      <td>
                        {a.qtd > 0
                          ? <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 100, background: 'rgba(34,197,94,0.1)', color: '#4ade80' }}>Em carteira</span>
                          : <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 100, background: 'rgba(255,255,255,0.05)', color: '#6B7280' }}>Zerado</span>
                        }
                      </td>
                      <td>
                        <button onClick={() => excluirAtivo(a.id, a.ticker)} className="inv-btn inv-btn-danger" style={{ fontSize: 11 }}>✕</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                    ...
                    {proventosDoMes.map(p => {
                      ...
                      return (
                        <tr key={p.id}>
                          ...
                          <td>{tributado ? <span style={{ fontSize: 11, color: '#FCA5A5' }}>15% na fonte</span> : <span style={{ fontSize: 11, color: '#4ade80' }}>Isento</span>}</td>
                          <td>
                            <div style={{ display: 'flex', gap: 4 }}>
          ...
          <div style={{ background: ir.totalVendas < 20000 ? 'rgba(34,197,94,0.08)' : 'rgba(245,158,11,0.08)', border: `1px solid ${ir.totalVendas < 20000 ? 'rgba(34,197,94,0.2)' : 'rgba(245,158,11,0.2)'}`, borderRadius: 12, padding: '14px 18px', marginBottom: 16, fontSize: 13 }}>
            {ir.totalVendas === 0
              ...
              : ir.totalVendas < 20000
                ? <span style={{ color: '#4ade80' }}>✅ Vendas totais de {fmt(ir.totalVendas)} — abaixo de R$20.000. Lucros com ações são isentos de IR neste mês.</span>
                : <span style={{ color: AMBER }}>⚠️ Vendas totais de {fmt(ir.totalVendas)} — acima de R$20.000. Gere o DARF até o último dia útil do mês seguinte.</span>
            }
          </div>
                  ...
                  <tbody>
                    {ir.detalhes.map(d => (
                      <tr key={d.ticker}>
                        ...
                        <td>
                          {d.lucro < 0 ? <span style={{ fontSize: 11, color: '#6B7280' }}>Prejuízo</span>
                            : d.isento ? <span style={{ fontSize: 11, color: '#4ade80' }}>Isento</span>
      ...
      {aba === 'notas' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {/* Checklist */}
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#fff', marginBottom: 16 }}>✅ Checklist</div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              <input
                value={novaNotaTxt}
                onChange={e => setNovaNotaTxt(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && adicionarCheckItem()}
                placeholder="Novo item..."
                style={{ ...inp, flex: 1 }}
              />
              <button onClick={adicionarCheckItem} disabled={salvandoNota || !novaNotaTxt.trim()}
                style={{ background: INDIGO, border: 'none', borderRadius: 8, padding: '0 16px', fontSize: 13, color: '#fff', cursor: 'pointer', opacity: (!novaNotaTxt.trim() || salvandoNota) ? 0.6 : 1 }}>
                +
              </button>
            </div>
            {checkItems.length === 0 ? (
              <p style={{ color: '#6B7280', fontSize: 13, textAlign: 'center', padding: '24px 0' }}>Nenhum item no checklist ainda.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 6 }}>
                {checkItems.map(item => (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)' }}>
                    <input type="checkbox" checked={item.feito} onChange={() => toggleCheckItem(item)}
                      style={{ width: 16, height: 16, accentColor: INDIGO, cursor: 'pointer', flexShrink: 0 }} />
                    <span style={{ flex: 1, fontSize: 13, color: item.feito ? '#4B5563' : '#E5E7EB', textDecoration: item.feito ? 'line-through' : 'none' }}>
                      {item.texto}
                    </span>
                    <button onClick={() => excluirCheckItem(item.id)} style={{ background: 'none', border: 'none', color: '#4B5563', cursor: 'pointer', fontSize: 14, padding: 2 }}>✕</button>
                  </div>
                ))}
                <div style={{ fontSize: 11, color: '#4B5563', marginTop: 4, textAlign: 'right' }}>
                  {checkItems.filter(c => c.feito).length}/{checkItems.length} concluídos
                </div>
              </div>
            )}
          </div>
          ...
          {/* Anotações livres */}
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#fff', marginBottom: 16 }}>📝 Anotações</div>
            <textarea
              value={anotacaoLivre}
              onChange={e => setAnotacaoLivre(e.target.value)}
              placeholder="Escreva suas estratégias, análises, ideias de aportes..."
              style={{ ...inp, minHeight: 220, resize: 'vertical', lineHeight: 1.6, fontFamily: 'inherit' }}
            />
            <button onClick={salvarAnotacao} disabled={salvandoAnotacao}
              style={{ marginTop: 10, background: INDIGO, border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 600, color: '#fff', cursor: 'pointer', opacity: salvandoAnotacao ? 0.7 : 1 }}>
              {salvandoAnotacao ? 'Salvando...' : '💾 Salvar anotações'}
            </button>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Ticker *</label>
              <input value={formAtivo.ticker} onChange={e => setFormAtivo(p => ({ ...p, ticker: e.target.value.toUpperCase() }))} placeholder="Ex: PETR4, MXRF11, BTC" style={inp} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              ...
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Nome (opcional)</label>
                <input value={formAtivo.nome} onChange={e => setFormAtivo(p => ({ ...p, nome: e.target.value }))} placeholder="Ex: Petrobras" style={inp} />
              </div>
            </div>

            {/* Posição inicial */}
            <div style={{ background: 'rgba(79,70,229,0.06)', border: '1px solid rgba(79,70,229,0.2)', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#818CF8', marginBottom: 12 }}>📊 Posição inicial (opcional)</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={{ fontSize: 12, color: '#9CA3AF', display: 'block', marginBottom: 5 }}>Quantidade</label>
                  <input type="number" min="0" step="any" value={formAtivo.qtdInicial || ''} onChange={e => setFormAtivo(p => ({ ...p, qtdInicial: parseFloat(e.target.value) || 0 }))} placeholder="0" style={inp} />
                </div>
                <div>
                  <label style={{ fontSize: 12, color: '#9CA3AF', display: 'block', marginBottom: 5 }}>Preço médio (R$)</label>
                  <input type="number" min="0" step="any" value={formAtivo.precoMedioInicial || ''} onChange={e => setFormAtivo(p => ({ ...p, precoMedioInicial: parseFloat(e.target.value) || 0 }))} placeholder="0,00" style={inp} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: 12, color: '#9CA3AF', display: 'block', marginBottom: 5 }}>Data de referência</label>
                <input type="date" value={formAtivo.dataInicial} onChange={e => setFormAtivo(p => ({ ...p, dataInicial: e.target.value }))} style={inp} />
              </div>
              {formAtivo.qtdInicial > 0 && formAtivo.precoMedioInicial > 0 && (
                <div style={{ marginTop: 10, fontSize: 12, color: '#818CF8' }}>
                  Custo total: <strong>{fmt(formAtivo.qtdInicial * formAtivo.precoMedioInicial)}</strong>
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={salvarAtivo} disabled={salvando} style={{ flex: 1, background: INDIGO, border: 'none', borderRadius: 8, padding: 12, fontSize: 14, fontWeight: 600, color: '#fff', cursor: 'pointer', opacity: salvando ? 0.7 : 1 }}>
        ...
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Ticker *</label>
                <input list="tickers-lista-pf" value={formOp.ticker} onChange={e => {
                  const ticker = e.target.value.toUpperCase()
                  const ativo = ativos.find(a => a.ticker === ticker)
                  setFormOp(p => ({ ...p, ticker, ativo_id: ativo?.id || '' }))
                }} placeholder="Ex: PETR4" style={inp} />
                <datalist id="tickers-lista-pf">{ativos.map(a => <option key={a.id} value={a.ticker} />)}</datalist>
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Tipo</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Quantidade *</label>
                <input type="number" min="0" step="any" value={formOp.quantidade || ''} onChange={e => setFormOp(p => ({ ...p, quantidade: parseFloat(e.target.value) || 0 }))} style={inp} />
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Preço unitário (R$) *</label>
                <input type="number" min="0" step="any" value={formOp.preco_unitario || ''} onChange={e => setFormOp(p => ({ ...p, preco_unitario: parseFloat(e.target.value) || 0 }))} style={inp} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                ...
                <input type="date" value={formOp.data} onChange={e => setFormOp(p => ({ ...p, data: e.target.value }))} style={inp} />
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Corretora</label>
                <input value={formOp.corretora} onChange={e => setFormOp(p => ({ ...p, corretora: e.target.value }))} placeholder="Ex: XP, Clear, Rico..." style={inp} />
              </div>
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Observações</label>
              <input value={formOp.obs} onChange={e => setFormOp(p => ({ ...p, obs: e.target.value }))} placeholder="Opcional..." style={inp} />
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
        ...
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Ticker *</label>
                <input list="tickers-prov-pf" value={formProv.ticker} onChange={e => setFormProv(p => ({ ...p, ticker: e.target.value.toUpperCase() }))} placeholder="Ex: MXRF11" style={inp} />
                <datalist id="tickers-prov-pf">{ativos.map(a => <option key={a.id} value={a.ticker} />)}</datalist>
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Tipo</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Valor por cota (R$) *</label>
                <input type="number" min="0" step="any" value={formProv.valor_por_cota || ''} onChange={e => setFormProv(p => ({ ...p, valor_por_cota: parseFloat(e.target.value) || 0 }))} style={inp} />
              </div>
              <div>
                <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Quantidade de cotas *</label>
                <input type="number" min="0" step="any" value={formProv.quantidade || ''} onChange={e => setFormProv(p => ({ ...p, quantidade: parseFloat(e.target.value) || 0 }))} style={inp} />
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 13, color: '#9CA3AF', display: 'block', marginBottom: 6 }}>Data de pagamento *</label>
              <input type="date" value={formProv.data_pagamento} onChange={e => setFormProv(p => ({ ...p, data_pagamento: e.target.value }))} style={inp} />
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
      ...
      {modalPreco && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}
          ...
          <div style={{ background: '#0D0F1A', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 360 }}>
            ...
            <p style={{ fontSize: 11, color: '#4B5563', textAlign: 'center', marginTop: 12 }}>* O preço é salvo apenas durante a sessão.</p>
          </div>
        </div>
      )}
    </div>
